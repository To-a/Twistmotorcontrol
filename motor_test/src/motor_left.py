...
class motor_command():
    ...
    def __init__(self):  # 初期化
        ...
        self.Enable = True
        self.pin_num_PWM = 17  # pinの名前
        self.pin_num_RotationDirect = 22
        self.pin_num_Enable = 14
        # self.pin_num_Stop = 22
        self.freq = 20000  # Hz (PWM のパルスを一秒間に 20kHz 個生成)
        # ノード立ち上げ（ROS）
        rospy.init_node('left_test')
    ...
    def callback(self, data):
        ...
        if duty_command >= 0:  # 正逆回転
            self.RotationDirect = True  # default True
        else:
            self.RotationDirect = False  # 逆回転default False
